...
recommend_indexes = random.sample(range(len(products)), 5)
for i in range(len(products)):
    if i in recommend_indexes:
        products[i]['recommend'] = True
    else:
        products[i]['recommend'] = False

with open("discounted_products.json", "w") as outfile:
